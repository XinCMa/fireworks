...
void drawFlickeringGradientBar(int pos, const ExplosionParams &params);

void setup() {
  Serial.begin(115200);
  Serial.println("==== Explosion Effect: Flickering Gradient Bar ====");
...
void explosionEffect(const ExplosionParams &params) {
  ...
  for (int pos = startPos; pos >= endPos; pos--) {
    drawFlickeringGradientBar(pos, params);
    FastLED.show();
...
/**
 * @brief 绘制一个随机闪烁的渐变条带
 *        条带从 color1 → color2 渐变，长度为 stripLen
 *        每个LED有50%的概率显示或不显示
 */
void drawFlickeringGradientBar(int pos, const ExplosionParams &params) {
  // 清空灯带
  ...
  // 在 [pos-stripLen+1, pos] 范围内绘制渐变条带
